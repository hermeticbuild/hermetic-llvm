...
load(
    ...
    "GCC_VERSIONS",
    "gcc_config_toolexeclibdir_target",
...
def _gcc_audit_data(version):
    return (
        _AUTOCONF_AUDIT_BASE_DATA +
        [gcc_repository_label(version, target) for target in _GCC_AUDIT_TARGETS] +
        [gcc_repository_label(version, gcc_config_toolexeclibdir_target(version))]
    )

def _gcc_audit_env(version):
    return {
        "GCC_ACINCLUDE": _rootpath(gcc_repository_label(version, "libstdc++-v3/acinclude.m4")),
        "GCC_CONFIG_ACX": _rootpath(gcc_repository_label(version, "config/acx.m4")),
        "GCC_CONFIG_CET": _rootpath(gcc_repository_label(version, "config/cet.m4")),
        "GCC_CONFIG_FUTEX": _rootpath(gcc_repository_label(version, "config/futex.m4")),
        "GCC_CONFIG_GCXXFILT": _rootpath(gcc_repository_label(version, "config/gc++filt.m4")),
        "GCC_CONFIG_GTHR": _rootpath(gcc_repository_label(version, "config/gthr.m4")),
        "GCC_CONFIG_HWCAPS": _rootpath(gcc_repository_label(version, "config/hwcaps.m4")),
        "GCC_CONFIG_ICONV": _rootpath(gcc_repository_label(version, "config/iconv.m4")),
        "GCC_CONFIG_LTHOSTFLAGS": _rootpath(gcc_repository_label(version, "config/lthostflags.m4")),
        "GCC_CONFIG_MULTI": _rootpath(gcc_repository_label(version, "config/multi.m4")),
        "GCC_CONFIG_NO_EXECUTABLES": _rootpath(gcc_repository_label(version, "config/no-executables.m4")),
        "GCC_CONFIG_TLS": _rootpath(gcc_repository_label(version, "config/tls.m4")),
        "GCC_CONFIG_TOOLEXECLIBDIR": _rootpath(gcc_repository_label(version, gcc_config_toolexeclibdir_target(version))),
        "GCC_CONFIG_UNWIND_IPINFO": _rootpath(gcc_repository_label(version, "config/unwind_ipinfo.m4")),
        "GCC_CONFIGURE_AC": _rootpath(gcc_repository_label(version, "libstdc++-v3/configure.ac")),
        "GCC_CONFIGURE_HOST": _rootpath(gcc_repository_label(version, "libstdc++-v3/configure.host")),
        "GCC_CROSSCONFIG": _rootpath(gcc_repository_label(version, "libstdc++-v3/crossconfig.m4")),
        "GCC_LINKAGE": _rootpath(gcc_repository_label(version, "libstdc++-v3/linkage.m4")),
        "GCC_VERSION": version,
