_LLVM_TARGET_TRIPLES = {
    ...
    "@llvm//platforms/config:windows_x86_64": "x86_64-pc-windows-msvc",
    "@llvm//platforms/config:windows_aarch64": "aarch64-pc-windows-msvc",
    ...
    "@llvm//platforms/config:windows_x86_64_gnullvm": "x86_64-pc-windows-gnullvm",
    "@llvm//platforms/config:windows_aarch64_gnullvm": "aarch64-pc-windows-gnullvm",
...
LLVM_TARGET_TRIPLE = select({
    platform: [triple]
    for platform, triple in _LLVM_TARGET_TRIPLES.items()
}, no_match_error = "Unsupported platform")

LLVM_TARGET_FLAG = select({
    platform: ["--target=%s" % triple]
    for platform, triple in _LLVM_TARGET_TRIPLES.items()
}, no_match_error = "Unsupported platform")
